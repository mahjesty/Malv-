...
import type { VoiceState } from './LivingOrbVisualizer'

type ScreenPhase = 'pre-call' | 'active' | 'ended'

const DEMO_STATES: VoiceState[] = ['listening', 'thinking', 'speaking', 'listening']
...
export default function CallScreen() {
  ...
  const [callState, setCallState] = useState<VoiceState>('listening')
  ...
  // Demo: animate audio level based on call state
  useEffect(() => {
    if (phase !== 'active') return
    const id = setInterval(() => {
      if (callState === 'muted') {
        setAudioLevel(0.008 + Math.random() * 0.008)
      } else if (callState === 'idle') {
        setAudioLevel(0.035 + Math.random() * 0.03)
      } else if (callState === 'thinking') {
        setAudioLevel(0.045 + Math.random() * 0.04)
      } else if (callState === 'listening') {
        setAudioLevel(0.09 + Math.random() * 0.1)
      } else {
        setAudioLevel(0.18 + Math.random() * 0.22)
      }
    }, 120)
    return () => clearInterval(id)
  }, [phase, callState])
  ...
  return (
    ...
    >
      {isMinimized ? (
        ...
        <>
          {/* Full-screen canvas orb — no container constraints */}
          <LivingOrbVisualizer 
            state={micMuted ? 'muted' : callState}
            audioLevel={audioLevel}
          />
