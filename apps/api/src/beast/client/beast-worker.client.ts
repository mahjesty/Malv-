...
export class BeastWorkerClient {
  ...
  async infer(args: BeastInferenceRequest): Promise<BeastInferenceResponse> {
    ...
    const url = `${baseUrl.replace(/\/$/, "")}/v1/infer`;
    ...
    this.logger.log(
      `[MALV E2E] worker infer request POST ${url} correlationId=${correlationId} mode=${args.mode} promptLen=${args.prompt.length}`
    );
    this.logger.log(
    ...
    try {
      res = await fetch(url, {
        ...
        body: JSON.stringify({ mode: args.mode, prompt: args.prompt, context: ctx }),
        signal: args.signal
      });
    } catch (e) {
      ...
      this.logger.error(`[MALV E2E] worker infer failed (fetch) ${msg}`);
      this.logger.error(`[MALV BRAIN] worker missing / queue idle (fetch failed): ${msg}`);
      throw new Error(msg || "Beast worker unreachable");
    }

    if (!res.ok) {
      const txt = await res.text().catch(() => "");
      this.logger.error(`[MALV E2E] worker infer failed HTTP ${res.status} bodyHead=${txt.slice(0, 120)}`);
      this.logger.error(`[MALV BRAIN] error in generation pipeline: HTTP ${res.status} ${txt.slice(0, 200)}`);
      throw new Error(txt || `Beast worker HTTP ${res.status}`);
    }
  ...
  async health(): Promise<{
    ...
    const baseUrl = this.cfg.get<string>("BEAST_WORKER_BASE_URL") ?? "http://127.0.0.1:9090";
    const apiKey = this.cfg.get<string>("BEAST_WORKER_API_KEY") ?? "";
    const url = `${baseUrl.replace(/\/$/, "")}/v1/health/inference`;
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      this.logger.warn(`[MALV WORKER] health check failed: ${msg}`);
      return {
        ok: false,
        reachable: false,
        inferenceConfigured: false,
